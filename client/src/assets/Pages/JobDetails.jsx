...
const JobDetails = () => {
  ...
  const fetchJobDetails = () => {
    fetch(`http://localhost:3000/job/${id}`)
      ...
      .then((data) => {
        setJob(data);
        // Set initial application status based on fetched data
        if (user && data.applicants?.some((app) => app.email === user.email)) {
          setHasApplied(true);
        } else {
          setHasApplied(false); // Explicitly set to false if not applied
        }
      })
      .catch((error) => console.error("Error fetching job details:", error));
  };

  // Effect to fetch job details when component mounts or user changes
  useEffect(() => {
    fetchJobDetails();
  }, [id, user]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setHasApplied(true); // Immediately update the button state
        // Optionally re-fetch to update applicant count
        fetchJobDetails();
      } else {
  ...
  const renderApplyButton = () => {
    ...
    if (isClosed) {
      return (
        <button disabled className="bg-gray-400 text-white font-bold py-2 px-6 rounded-full">
          Đã đóng
        </button>
      );
    }
    ...
    return (
      <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-full transition duration-300" onClick={handleApply}>
        Ứng tuyển ngay
      </button>
    );
  };
      ...
      <div className="py-10">
        <div className="bg-white p-8 rounded-lg shadow-md">
          <div className="flex justify-between items-start flex-wrap gap-4">
            ...
            <div className="text-right">{renderApplyButton()}</div>
          </div>
